...
export async function getAllReviews(): Promise<(ReviewDTO & { productId: string; productName: string })[]> {
  const supabase = await createClient();
  const { data } = await supabase
    .from("reviews")
    .select("id, author_name, rating, created_at, title, comment, is_verified, is_approved, product_id, product:products(name)")
    .order("created_at", { ascending: false });

  return ((data ?? []) as unknown as Array<Parameters<typeof toDTO>[0] & { product_id: string; product: { name: string } | null }>).map((r) => ({
    ...toDTO(r),
