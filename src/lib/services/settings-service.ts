...
export async function updateStoreSettings(
  data: Partial<Omit<StoreSettingsDTO, "id" | "freeShippingThreshold">> & { freeShippingThreshold?: number }
  ...
  const { data: updated } = await supabase.from("store_settings").update(patch).eq("id", "singleton").select("*").maybeSingle();
